...
public class UserAuthPublicKeyIterator extends AbstractKeyPairIterator<PublicKeyIdentity> implements Channel {
    ...
    public static Iterator<KeyPair> iteratorOf(ClientSession session) {
        return (session == null) ? Collections.<KeyPair>emptyIterator() : KeyIdentityProvider.iteratorOf(session.getRegisteredIdentities(), session.getKeyPairProvider());
    }
    ...
    public UserAuthPublicKeyIterator(ClientSession session, SignatureFactoriesManager signatureFactories) throws Exception {
        ...
        identities.add(new SessionKeyPairIterator(session, signatureFactories, iteratorOf(session)));
